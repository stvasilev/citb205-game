...
{
public:
    Graphics(HANDLE hNewScreenBuffer, int rows, int cols) : rows(rows), cols(cols), hNewScreenBuffer(hNewScreenBuffer)
    {
        buffer = new CHAR_INFO[rows * cols]{};
        coordBufSize.Y = this->rows;
        coordBufSize.X = cols;
        ...
        srctWriteRect.Bottom = this->rows - 1;
        srctWriteRect.Right = cols - 1;
    }
    void fillPixel(int x, int y)
    {
        buffer[y * cols + x].Char.AsciiChar = ' ';
        buffer[y * cols + x].Attributes = BACKGROUND_GREEN;
    }
